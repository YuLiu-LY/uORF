...
from util.vis_utils import rotate_view, rotate


class uorfVisModel(BaseModel):
    ...
    def set_input(self, input, thetas):
        ...
        view = input['cam2world'][0]
        cam2world = []
        for theta in thetas:
            view1 = rotate_view(view, theta)
            cam2world.append(view1)
        self.cam2world = torch.stack(cam2world, dim=0).type(torch.float32).to(self.device)
        # cam2world = input['cam2world'].to(self.device)
        # print('cam2world', cam2world)
        if not self.opt.fixed_locality:
            self.cam2world_azi = input['azi_rot'].to(self.device)
            # print('cam2world_azi', cam2world_azi[0])
    ...
    def forward(self, epoch=0):
        ...
        # Slot Attention
        z_slots, attn = self.netSlotAttention(feat, s=0)  # 1xKxC, 1xKxN
        ...
        with torch.no_grad():
            for i in range(self.opt.n_img_each_scene):
                ...
                if i == 0:
                    ...
                    setattr(self, 'gt_novel_view{}'.format(i), x[i % 4])
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            predefined_colors = gt_mask0.flatten(start_dim=1).unique(dim=1).T  # Kx3
            # swap the first color and the third color
            t = predefined_colors[0].clone()
            predefined_colors[0] = predefined_colors[2]
            predefined_colors[2] = t
            mask_visuals = predefined_colors[mask_idx]  # NxHxWx3
            mask_visuals = mask_visuals.permute([3, 0, 1, 2])  # 3xNxHxW
